...
function [par_gs, RMSE_GS, R2_GS, VPD_BINS_UP, GS_BINS_UP, ET_BINS_UP, Bo_BINS, EF_BINS] = boundary_line_analysis(lng, DOY, HH, H, LE, VPD, RN, SWIN, TA, GS, ga)
%%% input variables %%%
% lng = number of sites
...
for k=1:lng;
    ...
    % fit model for VPD-gs relationship
            ...
            if sum(~isnan(GS_UP))>10;
                ...
                par_gs(k,:) = nlinfit(x(i1),y(i1),gsFxn,par0_gs,opts);
                clear x y
                % goodness of fit
                x=VPD_BINS_UP(k,:);
                y=GS_BINS_UP(k,:);
                RMSE_GS(k)=sqrt(mean(y(~isnan(x) & ~isnan(y) & x>thr)-(gsFxn(par_gs(k,:),x(~isnan(x) &~isnan(y) & x>thr))./1000)).^2);
                R=corrcoef(y(~isnan(x) & ~isnan(y) & x>thr),(gsFxn(par_gs(k,:),x(~isnan(x) &~isnan(y) & x>thr))./1000));
                R2_GS(k)=R(1,2)^2;
